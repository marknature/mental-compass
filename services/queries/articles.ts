...
export async function getArticles(input: Partial<GetEventsSchema>) {
  try {
    const serialize = createSerializer(eventsParams);
    const params = serialize(input);

    const response = await axios.get<QueriedEvent[]>(`/api/articles${params}`);

    if (response.status !== 200) throw new Error("Failed to fetch articles");
